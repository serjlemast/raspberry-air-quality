...
public class HumiTempComponent extends Component {
  ...
  private static final String DEFAULT_HUMI_PATH =
      "/sys/devices/platform/dht11@4/iio:device0/in_humidityrelative_input";

  private static final String DEFAULT_TEMP_PATH =
      "/sys/devices/platform/dht11@4/iio:device0/in_temp_input";
  ...
  private static final int DEFAULT_POLLING_DELAY_MS = 1000;
  ...
  /**
   * Creates a new humidity and temperature sensor component with default paths and custom polling
   * interval
   *
  ...
  /**
   * (Re-)starts the poller with the desired time period in milliseconds. If the poller is already
   * running, it will be cancelled and rescheduled with the given time. The first poll happens
   * immediately in a separate thread and does not get delayed.
   *
  ...
  protected synchronized void startPoller(long pollerPeriodMs) {
    ...
    this.poller =
        scheduler.scheduleAtFixedRate(new Poller(), 0, pollerPeriodMs, TimeUnit.MILLISECONDS);
  }
  ...
  /**
   * Poller class which implements {@link Runnable} to be used with {@link ScheduledExecutorService}
   * for repeated execution. This poller consecutively starts reads the values in the humidity and
   * temperature files
   */
